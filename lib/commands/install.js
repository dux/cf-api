...
async function checkAuth() {
  ...
  // check where env vars are loaded from
  const cwdEnv = resolve(process.cwd(), '.env')
  const homeEnv = resolve(homedir(), '.env')
  if (existsSync(cwdEnv)) info('.env', `loaded from ${cwdEnv}`)
  else if (existsSync(homeEnv)) info('.env', `loaded from ${homeEnv}`)
...
function checkConfig() {
  ...
  if (existsSync(envFile)) {
    ok('.env', 'found (loaded at startup)')
  }
}
...
function printNotes() {
  ...
  gap()
  line(`Vars can go in ${C}~/.env${X} or ${C}./.env${X} - loaded automatically.`)
  ...
  line(`- .env files loaded from cwd first, then ~/.env`)
